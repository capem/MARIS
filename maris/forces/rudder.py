...
class RudderForce:
    ...
    def compute(self, state: VesselState, control: ControlInput, env: EnvironmentSample, params: VesselParams) -> Dict[str, float]:
        ...
        eff = float(rp.get("eff_factor", 1.2))  # slightly >1 to amplify placeholder effect

        rho = params.rho_water if math.isfinite(getattr(params, "rho_water", 1025.0)) else 1025.0
