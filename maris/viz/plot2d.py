...
def plot_run_csv(
    ...
    if options.color_by_speed:
        sc = ax.scatter(
            x, y, c=speed, s=8, cmap="viridis", alpha=0.9, edgecolors="none"
        )
    ...
    if options.show_heading_ticks:
        ...
        L = 0.05 * max(
            1.0, float(np.nanmax(np.hypot(x - np.nanmin(x), y - np.nanmin(y))) or 1.0)
        )
        hx = np.cos(psi[idx]) * L
        hy = np.sin(psi[idx]) * L
        ax.quiver(
    ...
    if (
        "wind_speed" in data
        and "wind_dir_from" in data
        and np.any(np.isfinite(data["wind_speed"]))
    ):
    ...
    if (
        "current_speed" in data
        and "current_dir_to" in data
        and np.any(np.isfinite(data["current_speed"]))
    ):
        stride = max(1, int(options.quiver_stride))
